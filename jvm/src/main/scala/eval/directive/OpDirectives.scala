...
import geotrellis.raster.{isData, Tile}
...
object OpDirectives {
   ...
   */
  val additionDouble = Directive {
    case (a @ Addition(_), childResults) if a.kind == MamlKind.Double =>
      childResults
        .map { _.as[Double] }
        .toList
        .sequence
  ...
  val additionInt = Directive {
    case (a @ Addition(_), childResults) if a.kind == MamlKind.Int =>
      childResults
        .map { _.as[Int] }
        .toList
        .sequence
  ...
  val additionTile = Directive {
    case (a @ Addition(_), childResults) if a.kind == MamlKind.Image =>
  ...
  val subtraction = Directive { case (a @ Subtraction(_), childResults) =>
    val results = childResults.reduce { (res1, res2) =>
      tileOrScalarReduction({ _ - _ }, { _ -: _ }, { _ - _ }, { _ -: _ }, { _ - _ }, { _ - _ }, { _ - _ }, { _ - _ }, { _ - _ }, res1, res2)
    }
    Valid(results)
  }
  ...
  val multiplicationDouble = Directive {
    case (a @ Multiplication(_), childResults) if a.kind == MamlKind.Double =>
      childResults
        .map { _.as[Double] }
        .toList
        .sequence
  ...
  val multiplicationInt = Directive {
    case (a @ Multiplication(_), childResults) if a.kind == MamlKind.Double =>
      childResults
        .map { _.as[Double] }
        .toList
  ...
  val multiplicationTile = Directive {
    case (a @ Multiplication(_), childResults) if a.kind == MamlKind.Image =>
      val grouped = childResults.groupBy(_.kind)
   ...
   */
  val maxDouble = Directive {
    case (a @ Max(_), childResults) if a.kind == MamlKind.Double =>
      childResults
        .map { _.as[Double] }
        .toList
        .sequence
  ...
  val maxTile = Directive {
    case (a @ Max(_), childResults) if a.kind == MamlKind.Image =>
      val grouped = childResults.groupBy(_.kind)
  ...
  val rgbTile = Directive {
    case (a @ RGB(_, rb, gb, bb), childResults) if a.kind == MamlKind.Image =>
      val grouped = childResults.groupBy(_.kind)
      ...
      imageResults(grouped).map { tiles =>
        tiles.take(3) match {
          ...
          case list =>
            ImageResult(list.reduce { (lt1: LazyMultibandRaster, lt2: LazyMultibandRaster) =>
  ...
  val assembleTile = Directive {
    case (a @ Assemble(_), childResults) if a.kind == MamlKind.Image =>
      val grouped = childResults.groupBy(_.kind)
  ...
  val minDouble = Directive {
    case (a @ Min(_), childResults) if a.kind == MamlKind.Double =>
      childResults
        .map { _.as[Double] }
        .toList
        .sequence
  ...
  val minTile = Directive {
    case (a @ Min(_), childResults) if a.kind == MamlKind.Image =>
      val grouped = childResults.groupBy(_.kind)
