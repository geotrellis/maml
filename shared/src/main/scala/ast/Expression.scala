...
/**
 * It is a [[FoldableExpression]] though expects only 3 rasters to be passed. It would use the first band of every raster and combine them into a
 * single RGB raster.
 *
 * redBand - the nam / number of the band from the first (red) argument greenBand - the name / number of the band from the second (green) argument
 * blueBand - the name / number of the band from the third (blue) argument
 */
case class RGB(children: List[Expression], redBand: String = "0", blueBand: String = "0", greenBand: String = "0")
    extends Expression("rgb")
...
case class Branch(children: List[Expression]) extends Expression("ifelse") {
  ...
  require(
    ...
    s"Unable to determine branching/if-else kind. If and Else body must be of the same kind. If-body: ${children(1).kind}. Else-body: ${children(2).kind}"
  )
...
case class Rescale(children: List[Expression], newMin: Double, newMax: Double, band: Option[String] = None)
    extends Expression("rescale")
    with UnaryExpression {
  val kindDerivation: Map[MamlKind, MamlKind] = UnaryExpression.imageOrScalar
  def withChildren(newChildren: List[Expression]): Expression = copy(children = newChildren)
}

case class Normalize(children: List[Expression], oldMin: Double, oldMax: Double, newMin: Double, newMax: Double, band: Option[String] = None)
    extends Expression("normalize")
    with UnaryExpression {
  val kindDerivation: Map[MamlKind, MamlKind] = UnaryExpression.imageOrScalar
  def withChildren(newChildren: List[Expression]): Expression = copy(children = newChildren)
}
...
case class FocalMax(children: List[Expression], neighborhood: Neighborhood, target: TargetCell = TargetCell.All)
    extends Expression("fmax")
    with FocalExpression {
  def withChildren(newChildren: List[Expression]): Expression = copy(children = newChildren)
}

case class FocalMin(children: List[Expression], neighborhood: Neighborhood, target: TargetCell = TargetCell.All)
    extends Expression("fmin")
    with FocalExpression {
  def withChildren(newChildren: List[Expression]): Expression = copy(children = newChildren)
}

case class FocalMean(children: List[Expression], neighborhood: Neighborhood, target: TargetCell = TargetCell.All)
    extends Expression("fmean")
    with FocalExpression {
  def withChildren(newChildren: List[Expression]): Expression = copy(children = newChildren)
}

case class FocalMedian(children: List[Expression], neighborhood: Neighborhood, target: TargetCell = TargetCell.All)
    extends Expression("fmedian")
    with FocalExpression {
  def withChildren(newChildren: List[Expression]): Expression = copy(children = newChildren)
}

case class FocalMode(children: List[Expression], neighborhood: Neighborhood, target: TargetCell = TargetCell.All)
    extends Expression("fmode")
    with FocalExpression {
  def withChildren(newChildren: List[Expression]): Expression = copy(children = newChildren)
}

case class FocalSum(children: List[Expression], neighborhood: Neighborhood, target: TargetCell = TargetCell.All)
    extends Expression("fsum")
    with FocalExpression {
  def withChildren(newChildren: List[Expression]): Expression = copy(children = newChildren)
}

case class FocalStdDev(children: List[Expression], neighborhood: Neighborhood, target: TargetCell = TargetCell.All)
    extends Expression("fstddev")
    with FocalExpression {
  def withChildren(newChildren: List[Expression]): Expression = copy(children = newChildren)
}

case class FocalSlope(children: List[Expression], zFactor: Option[Double] = None, target: TargetCell = TargetCell.All)
    extends Expression("fslope")
    with FocalExpression {
...
case class FocalHillshade(children: List[Expression],
                          azimuth: Double,
                          altitude: Double,
                          zFactor: Option[Double] = None,
                          target: TargetCell = TargetCell.All
) extends Expression("fhillshade")
    with FocalExpression {
  // Not used in this focal operation
  def neighborhood = Square(1)
  def withChildren(newChildren: List[Expression]): Expression = copy(children = newChildren)
